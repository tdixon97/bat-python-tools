...
with open(cfg_path,"r") as file_cfg:
...
data_counts = utils.get_data_counts_total("mul_surv",det_types,regions,file,key_list=[name_peak])
data_counts_right = utils.get_data_counts_total("mul_surv",det_types,right_sideband,file,key_list=[name_peak])
data_counts_left = utils.get_data_counts_total("mul_surv",det_types,left_sideband,file,key_list=[name_peak])

data_counts_lar = utils.get_data_counts_total("mul_lar_surv",det_types,regions,file,key_list=[name_peak])
data_counts_lar_right = utils.get_data_counts_total("mul_lar_surv",det_types,right_sideband,file,key_list=[name_peak])
data_counts_lar_left = utils.get_data_counts_total("mul_lar_surv",det_types,left_sideband,file,key_list=[name_peak])
...
for key in sorted(effs["all"][name_peak].keys()):
    if ((name_peak=="Tl_peak" and "Tl208" in key)
        or (name_peak=="Bi_peak" and "Bi214" in key)):
        eff_tmp  =eff_map[key]
...
axes.set_title("LAr survival for {} ( {} keV)".format(name_peak,peak))
