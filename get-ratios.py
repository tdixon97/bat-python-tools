...
def project_gamma_line(
    ...
    variable_select="e2",
    variable_project="e1",
    title="Projection of $E_2$ 609 keV",
    counting_rates={},
    name="m2_project",
    plot=False,
    just_value=False,
    ...
    bin_edges_1 = variable_binning.compute_binning(
        gamma_energies=gamma_list,
    ...
    if variable_project == "e1":
        ...
        idx = 0
    histo_projection_1 = utils.select_region(
        histo,
        [
            {"var": variable_select, "greater": False, "val": peak_energy[0] + peak_energy[1]},
            {"var": variable_select, "greater": True, "val": peak_energy[0] - peak_energy[1]},
        ],
    ).project(idx)
    ...
    for p in gamma_sig:
        ...
        counts = np.array(
            [
                utils.integrate_hist(histo_projection_1, p - 30, p - 5),
...
def main():
    def get_ratio(a, b):
        ...
        if a[0] > 0 and b[0] > 0:
            r = 100 * a[0] / b[0]
            return r, np.sqrt(
                r * r * b[1] * b[1] / (b[0] * b[0]) + r * r * a[1] * a[1] / (a[0] * a[0])
            )
        if b[0] == 0:
            return 0, 100
    ...
    if plot_categories_1D:
        ...
        histo_a = utils.select_region(
            histo,
            [
                {"var": "e2", "greater": False, "val": 500},
                {"var": "sum", "greater": False, "val": 1400},
            ],
        )
        histo_b = utils.select_region(
            histo,
            [
                {"var": "e2", "greater": False, "val": 500},
                {"var": "sum", "greater": True, "val": 1400},
                {"var": "e1", "greater": False, "val": 1500},
            ],
        )
        histo_c = utils.select_region(
            histo,
            [
                {"var": "e2", "greater": False, "val": 500},
                {"var": "e1", "greater": True, "val": 1500},
            ],
        )
    ...
    if make_2D_plots:
        ...
        for h, n in zip(histos, names):
            ...
            if plot_regions == False:
                plt.savefig(f"plots/mult_two/2D_spec_{n}.png")
            else:
                plt.savefig(f"plots/mult_two/2D_spec_{n}_plus_regions.png")
            plt.show()
    ...
    if get_from_data:
        ...
        if plot_data_1D:
            ...
            axes_2.set_ylabel(f"counts/ {bin} keV")
            legend = axes_2.legend(
                loc="upper right", edgecolor="black", frameon=False, facecolor="white", framealpha=1
            )
            plt.show()
        ...
        for h, n in zip(histos, names):
            ...
            for s in spectra:
                ...
                for p in peaks:
                    ...
                    counts = np.array(
                        [
                            utils.integrate_hist(h1D, p - 20, p - size),
        ...
        project_gamma_line(
            ...
            "e1",
            "e2",
            "Projection of $E_1$ 2615 keV",
            data_counts,
            "mul2_surv_e1_2615",
            name_comb="Tl_2615_E1",
        )
            ...
            for spec in data_counts[cat]:
                for peak in data_counts[cat][spec]:
                    if np.isin(peak, gamma_list_Tl):
                        ratios_data[cat][spec][peak] = get_ratio(
                            data_counts[cat][spec][peak], data_counts["all"]["mul_surv"]["2615"]
                        )
                    else:
                        ratios_data[cat][spec][peak] = get_ratio(
                            data_counts[cat][spec][peak], data_counts["all"]["mul_surv"]["1764"]
                        )

        sum_3125_3197 = (
            data_counts["all"]["mul_surv"]["3125"][0] + data_counts["all"]["mul_surv"]["3197"][0]
        )
        sum_err_3125_3197 = np.sqrt(
            np.power(data_counts["all"]["mul_surv"]["3125"][0], 2)
        ...
        ratios_data["all"]["mul_surv"]["bi_sum"] = get_ratio(
            (bi_sum, bi_sum_err), data_counts["all"]["mul_surv"]["1764"]
        )

        ratios_data["all"]["mul_surv"][str(3125 + 3197)] = get_ratio(
            (sum_3125_3197, sum_err_3125_3197), data_counts["all"]["mul_surv"]["2615"]
        )
    ...
    #### --------------------------------
        ...
        with open(gamma_line_M1_path) as json_file:
            gamma_line_M1 = json.load(json_file)

        with open(gamma_line_Bi_path) as json_file:
            gamma_line_Bi = json.load(json_file)

        with open(gamma_line_groups_path) as json_file:
            gamma_line_groups = json.load(json_file)

        with open(gamma_line_M2_path) as json_file:
            gamma_line_M2 = json.load(json_file)

        with open(gamma_line_3MeV_M1_path) as json_file:
            gamma_line_3MeV_M1 = json.load(json_file)

        with open(gamma_line_3MeV_M2_path) as json_file:
            gamma_line_3MeV_M2 = json.load(json_file)

        with open(gamma_line_M2_cats) as json_file:
            gamma_line_M2_cats = json.load(json_file)

        ratios_data["all"]["mul_surv"]["583"] = quantiles_to_sym_error(
            gamma_line_M1["All"]["raw"]["583_to_2614"]
        )
        ratios_data["all"]["mul_surv"]["609"] = quantiles_to_sym_error(
            gamma_line_M1["All"]["raw"]["609_to_1764"]
        )
        ratios_data["all"]["mul_surv"][str(3125 + 3197)] = quantiles_to_sym_error(
            gamma_line_3MeV_M1["All"]["raw"]["3126M1_plus_3198M1_over_2615M1"]
        )

        ### new ratios
        ratios_data["all"]["mul_surv"]["1377"] = quantiles_to_sym_error(
            gamma_line_Bi["All"]["raw"]["1378_to_1764"]
        )
        ratios_data["all"]["mul_surv"]["1729"] = quantiles_to_sym_error(
            gamma_line_Bi["All"]["raw"]["1730_to_1764"]
        )
        ratios_data["all"]["mul_surv"]["2017"] = quantiles_to_sym_error(
            gamma_line_Bi["All"]["raw"]["2017_to_1764"]
        )
        ratios_data["all"]["mul_surv"]["1274"] = quantiles_to_sym_error(
            gamma_line_Bi["All"]["raw"]["1275_to_1764"]
        )
        ratios_data["all"]["mul_surv"]["1415"] = quantiles_to_sym_error(
            gamma_line_Bi["All"]["raw"]["1416_to_1764"]
        )

        ratios_data["all"]["mul2_sum"]["2615"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["2615M2sum_over_2615M1"]
        )
        ratios_data["all"]["mul2_sum"]["1764"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["1764M2sum_over_1764M1"]
        )

        ratios_data["all"]["mul2_e1"]["2615"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["2615M2E1_over_2615M1"]
        )
        ratios_data["all"]["mul2_e1"]["583"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["583M2E1_over_2615M1"]
        )
        ratios_data["all"]["mul2_e1"]["609"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["609M2E1_over_1764M1"]
        )
        ratios_data["all"]["mul2_e2"]["583"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["583M2E2_over_2615M1"]
        )
        ratios_data["all"]["mul2_e2"]["609"] = quantiles_to_sym_error(
            gamma_line_M2["All"]["raw"]["609M2E2_over_1764M1"]
        )

        ratios_data["cat_1"]["mul2_e1"]["2615"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["2615M2cat1_over_2615M2all_e1"]
        )
        ratios_data["cat_2"]["mul2_e1"]["2615"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["2615M2cat2_over_2615M2all_e1"]
        )
        ratios_data["cat_3"]["mul2_e1"]["2615"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["2615M2cat3_over_2615M2all_e1"]
        )

        ratios_data["cat_1"]["mul2_e1"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat1_over_583M2all_e1"]
        )
        ratios_data["cat_2"]["mul2_e1"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat2_over_583M2all_e1"]
        )
        ratios_data["cat_3"]["mul2_e1"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat3_over_583M2all_e1"]
        )

        ratios_data["cat_1"]["mul2_e1"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat1_over_609M2all_e1"]
        )
        ratios_data["cat_2"]["mul2_e1"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat2_over_609M2all_e1"]
        )
        ratios_data["cat_3"]["mul2_e1"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat3_over_609M2all_e1"]
        )

        ratios_data["cat_1"]["mul2_e2"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat1_over_583M2all_e2"]
        )
        ratios_data["cat_2"]["mul2_e2"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat2_over_583M2all_e2"]
        )
        ratios_data["cat_3"]["mul2_e2"]["583"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["583M2cat3_over_583M2all_e2"]
        )

        ratios_data["cat_1"]["mul2_e2"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat1_over_609M2all_e2"]
        )
        ratios_data["cat_2"]["mul2_e2"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat2_over_609M2all_e2"]
        )
        ratios_data["cat_3"]["mul2_e2"]["609"] = quantiles_to_sym_error(
            gamma_line_M2_cats["All"]["raw"]["609M2cat3_over_609M2all_e2"]
        )
        ...
        for group in ["top", "mid_top", "mid", "mid_bottom", "bottom"]:
            ...
            for peak in ["583.0", "609.0", "1765.0", "2615.0", "1461.0", "1525.0"]:
                data_counts[group]["mul_surv"][str(int(float(peak)))] = quantiles_to_sym_error(
                    gamma_line_groups[group]["raw"][peak], 1
                )
    ...
    gamma_list_E1 = np.array(
        [277, 511, 583, 609, 768, 934, 1120, 1173, 1238, 1333, 1408, 1525, 2104, 2615]
    )
    binning_E2 = variable_binning.compute_binning(
        gamma_energies=gamma_list_E2,
        low_energy=0,
        high_energy=1500,
        gamma_binning_low=10,
        gamma_binning_high=10,
    ...
    binning_E1 = variable_binning.compute_binning(
        gamma_energies=gamma_list_E1,
        low_energy=0,
        high_energy=3000,
        gamma_binning_low=10,
        gamma_binning_high=10,
    ...
    if plot_data_1D:
        ...
        for spec, spec_name in zip([0, 1, 2], ["Energy 2", "Energy 1", "Energy 1 + Energy 2"]):
            fig_3, axes_3 = lps.subplots(
                1, 1, figsize=(4, 2), sharex=True, gridspec_kw={"hspace": 0}
            )
            ...
            axes_3.set_ylabel(f"counts/ {bin} keV")
            legend = axes_3.legend(
                loc="upper right", edgecolor="black", frameon=False, facecolor="white", framealpha=1
            )
            plt.show()
    ...
    with open(priors_k42_file) as file_cfg:
            ...
            for cat in cats:
                if cat in m2_cats:
                    ...
                    with uproot.open(pdf_path + file, object_cache=None) as f:
                        if cat == "all":
                            mc[name][cat]["mul2_e2"] = (
                                f["mul2_surv_2d"]["all"].to_hist()[0:3300, 0:3300].project(0)
                            )
                            mc[name][cat]["mul2_e1"] = (
                                f["mul2_surv_2d"]["all"].to_hist()[0:3300, 0:3300].project(1)
                            )
                            histo_mc = f["mul2_surv_2d"][cat].to_hist()[0:3300, 0:3300]

                        else:
                            mc[name][cat]["mul2_e2"] = (
                                f["mul2_surv_2d"][cat].to_hist()[0:3300, 0:3300].project(0)
                            )
                            mc[name][cat]["mul2_e1"] = (
                                f["mul2_surv_2d"][cat].to_hist()[0:3300, 0:3300].project(1)
                            )
                            histo_mc = f["mul2_surv_2d"][cat].to_hist()[0:3300, 0:3300]
                ...
                for peak in peaks:
                    for spec in ["mul_surv", "mul2_e2", "mul2_e1"]:
                        ...
                        if peak in [1274, 1415]:
                            pdf_tmp = mc[name][cat][spec][(peak - 6) * 1j : (peak + 6) * 1j][
                                hist.rebin(4)
                            ]
                        else:
                            pdf_tmp = mc[name][cat][spec][(peak - 15) * 1j : (peak + 15) * 1j][
                                hist.rebin(10)
                            ]
                ...
                if plot_mc:
                    fig, axes = lps.subplots(
                        1, 1, figsize=(3, 2), sharex=True, gridspec_kw={"hspace": 0}
                    )
                    ...
                    if "Ac228" in file:
                        ...
                        high = 2000
                    elif ("Pb208" in file) or ("K42" in file):
                    ...
                    plt.savefig(f"plots/mult_two/mc/{file[0:-5]}_{cat}.png")
                    plt.close()
                    ...
                    plt.savefig(f"plots/mult_two/mc/1D_{file[0:-5]}_{cat}.pdf")
                    ...
                    for peak in mc_counts[comp][cat][spec]:
                        if "Pb214" in comp or "Po214" in comp:
                            ...
                            if (cat in m1_cats) and (spec == "mul_surv"):
                                ratios[comp]["all"]["mul_surv"][str(3197 + 3125)] = (
                                    100
                                    * (
                                        mc_counts[comp]["all"]["mul_surv"]["3197"]
                                        + mc_counts[comp]["all"]["mul_surv"]["3125"]
                                    )
                                    / norm
                                )
                        else:
    ...
    ### read from a file
        ...
        with open(save_mc) as json_file:
            mc_counts = json.load(json_file)

        with open(save_mc_ratio) as json_file:
            ...
            for spec in ratios_data[cat]:
                for peak in ratios_data[cat][spec]:
                    ...
                    for comp in list_of_mc:
                        if (spec in ratios[comp["name"]][cat]) and (
                            peak in ratios[comp["name"]][cat][spec]
                        ):
                            ratio_tmp[comp["name"][11:]] = ratios[comp["name"]][cat][spec][peak]
                            if (("Po214") in comp["name"]) or ("Pb214" in comp["name"]):
                                denom = "N(1764, M1)"
                    ...
                    if peak == str(3197 + 3125):
                        utils.plot_relative_intensities(
                            ...
                            data_val[1],
                            orders,
                            rf"$\frac{{N({3125}, {spec_name})+N({3197}, {spec_name})}}{{{denom}}}$",
                            f"plots/mult_two/ratio_{3125}_{3197}_{spec}_{cat}{extra}.pdf",
                        )
                    elif peak == "bi_sum":
                        utils.plot_relative_intensities(
                            ratio_tmp,
                            ...
                            data_val[1],
                            orders,
                            rf"$\frac{{N(1729+ \ldots +{spec_name})}}{{{denom}}}$",
                            f"plots/mult_two/ratio_{spec}_{cat}{extra}.pdf",
                        )
                    elif peak == "Bi":
                        utils.plot_relative_intensities(
                            ratio_tmp,
                            ...
                            data_val[1],
                            orders,
                            rf"$\frac{{N(\text{{Bi double coinc.}}, M2 )}}{{{denom}}}$",
                            f"plots/mult_two/ratio_Bi_{spec}_{cat}{extra}.pdf",
                        )
                    elif peak == "Tl_2615_E1":
                        utils.plot_relative_intensities(
                            ratio_tmp,
                            data_val[0],
                            data_val[1],
                            orders,
                            rf"$\frac{{N(\text{{Tl double coinc.}}, M2 )}}{{{denom}}}$",
                            f"plots/mult_two/ratio_Tl_{spec}_{cat}{extra}.pdf",
                        )
                        ...
                        utils.plot_relative_intensities(
                            ...
                            data_val[1],
                            orders,
                            rf"$\frac{{N({peak}, {spec_name})}}{{{denom}}}$",
                            f"plots/mult_two/ratio_{peak}_{spec}_{cat}{extra}.pdf",
                        )
    ...
    if plot_categories:
        ...
        for cat, name in zip(
            ["cat_1", "cat_2", "cat_3"], ["same-str-adj", "same-str-not-adj", "diff-str"]
        ):
            for peak in ["609", "583", "2615"]:
                for spec in ["mul2_e1", "mul2_e2"]:
                    if peak not in ratios_data[cat][spec]:
                        continue
                    ...
                    for comp in list_of_mc:
                        if (spec in ratios[comp["name"]][cat]) and (
                            ...
                            a = (
                                mc_counts[comp["name"]][cat][spec][peak],
                                np.sqrt(mc_counts[comp["name"]][cat][spec][peak]),
                            )
                            b = (
                                mc_counts[comp["name"]]["all"][spec][peak],
                                np.sqrt(mc_counts[comp["name"]]["all"][spec][peak]),
                            )
                            if b[0] == 0:
                                continue
                    ...
                    utils.plot_relative_intensities(
                        ...
                        data_val[1],
                        orders,
                        rf"$\frac{{N({peak}, {spec_name}, \text{{{name}}})}}{{{denom}}}$",
                        f"plots/mult_two/ratio_{peak}_{spec}_{cat}_to_all{extra}.pdf",
                    )
        ...
        for peak, dc, dc_spec, spec in zip(
            ["609", "583", "2615"],
            ["Bi", "Tl_583_E2", "Tl_2615_E1"],
            ["mul2_surv_e2_609", "mul2_surv_e2_583", "mul2_surv_e1_2615"],
            ["mul2_e2", "mul2_e2", "mul2_e1"],
        ):
            if peak not in data_counts["all"][spec]:
                continue
            ...
            for comp in list_of_mc:
                if (spec in ratios[comp["name"]][cat]) and (
                    ...
                    a = (
                        mc_counts[comp["name"]][cat][dc_spec][dc],
                        np.sqrt(mc_counts[comp["name"]][cat][dc_spec][dc]),
                    )
                    b = (
                        mc_counts[comp["name"]]["all"][spec][peak],
                        np.sqrt(mc_counts[comp["name"]]["all"][spec][peak]),
            ...
            utils.plot_relative_intensities(
                ...
                orders,
                r"$\frac{{N({}, {}, \text{{{}}})}}{{{}}}$".format(
                    peak, spec_name, "double coinc.", denom
                ),
                "plots/mult_two/ratio_{}_{}_{}_to_all{}.pdf".format(peak, spec, "DC", extra),
            )
    ...
    for c, s, p in zip(cats_tmps, specs_tmps, peaks_tmp):
        ...
        ratios_tt.append(rt)
        data_tt.append(dt)
        name_tt.append(rf"$\frac{{N({p}, {st})}}{{{de_t}}}$")

    utils.plot_relative_intensities_triptych(
        ratios_tt,
        [data_tt[0][0], data_tt[1][0], data_tt[2][0]],
        [data_tt[0][1], data_tt[1][1], data_tt[2][1]],
        orders,
        name_tt,
        "plots/mult_two/583_tryptich.pdf",
    )
    ...
    utils.plot_relative_intensities_triptych(
        ...
        orders,
        name_tt,
        "plots/mult_two/609_tryptich.pdf",
    )
    plt.show()
        ...
        for peak in ["583", "2615", "1525"]:
            ...
            for i in range(len_k):
                ...
                if i == 0:
                    ...
                    axes.axvspan(
                        xmin=i, xmax=i + 1, alpha=0.3, ymin=ymin, ymax=ymax, color=vset.orange
                    )
            ...
            for comp in orders_tmp:
                ...
                for i in range(len_k):
                    hist_z_tmp[i] = (
                        mc_counts[iso + "_" + comp][groups[i]]["mul_surv"][peak]
                        / gr[groups[i]]["exposure"]
                    )
                    mc_tot += (
                        mc_counts[iso + "_" + comp][groups[i]]["mul_surv"][peak]
                        / gr[groups[i]]["exposure"]
