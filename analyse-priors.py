...
binning = variable_binning.compute_binning(gamma_energies=np.array([583,609,911,1461,1525,2204,2615]),low_energy=500,high_energy=4000,gamma_binning_high=10,gamma_binning_low=10,cont_binning=10)
...
with uproot.open(ref_fit) as f:
    for key in f[ref_ds]["originals"].keys():
        ...
        if ("Bi" in key) or ("Ac" in key) or ("K40" in key) or ("fitted_data" in key) or ("total_model" in key):
            continue

        if (total_other is None):
            total_other= utils.get_hist_variable(f[ref_ds]["originals"][key],bins=binning,range=(0,4000))
        else:
            total_other+=utils.get_hist_variable(f[ref_ds]["originals"][key],bins=binning,range=(0,4000))
    ...
    for comp, info in components.items():
        ...
        for name in info["hists"]:
            ...
            if name not in f[ref_ds]["originals"]:
                    continue
            if hs[comp] is None:
                hs[comp] = utils.get_hist_variable(f[ref_ds]["originals"][name],bins=binning,range=(0,4000))
            else:
                hs[comp] += utils.get_hist_variable(f[ref_ds]["originals"][name],bins=binning,range=(0,4000))
...
with PdfPages('plots/priors/bin_contents.pdf') as pdf:
    ...
    for bin in range(len(output[:,0])):
            ...
            lows.append(low)
            highs.append(high)
            if (abs(energies[bin]-2615)<5 or abs(energies[bin]-583)<5 or 
                abs(energies[bin]-1764)<5 or abs(energies[bin]-2204)<5 or 
                abs(energies[bin]-911)<5 or (abs(energies[bin]-1461)<5)
                ):
                fig, axes = lps.subplots(1, 1,figsize=(6, 4), sharex=True, gridspec_kw = { "hspace":0})
...
effs_right  = utils.get_total_efficiency(det_types,None,"mul_surv",right,pdf_path,mc_list=mc_list)
...
counts_911_data,low_911_data,high_911_data =utils.get_peak_counts(911,"Ac228",data_file,livetime)
counts_1461_data,low_1461_data,high_1461_data =utils.get_peak_counts(1461,"K40",data_file,livetime)
...
print("We see in 911  keV {} - {} + {} counts".format(counts_911_data,low_911_data,high_911_data))
print("We see in 1461  keV {} - {} + {} counts".format(counts_1461_data,low_1461_data,high_1461_data))
        ...
        for region in effs["all"].keys():
            ...
            for comp in  priors["components"]:
                if (type in comp["name"]):
                    ...
                    eff-=(effs_left["all"][region][comp["name"]]+effs_right["all"][region][comp["name"]])/2.
            ...
            utils.make_error_bar_plot(np.arange(len(labels)),labels,y,np.abs(y_low),np.abs(y_high),obj=region,categories=categories,data_band=data_band)
...
with PdfPages('plots/priors/shapes.pdf') as pdf:
    ...
    utils.compare_mc(583j,pdfs,"Bi212Tl208",order,2615j,pdf,500,700,"linear",linewidth=.8,colors=my_colors)
    utils.compare_mc(727j,pdfs,"Bi212Tl208",order,2615j,pdf,700,900,"linear",linewidth=.8,colors=my_colors)
    utils.compare_mc(2104j,pdfs,"Bi212Tl208",order,2615j,pdf,1800,2700,"linear",colors=my_colors)
    utils.compare_mc(100j,pdfs,"Pb214Bi214",order,1764j,pdf,0,4000,"log",colors=my_colors)
    utils.compare_mc(609j,pdfs,"Pb214Bi214",order,1764j,pdf,500,1000,"linear",linewidth=.8,colors=my_colors)
    utils.compare_mc(1764j,pdfs,"Pb214Bi214",order,1764j,pdf,1000,1300,"linear",linewidth=.8,colors=my_colors)

    utils.compare_mc(1764j,pdfs,"Pb214Bi214",order,1764j,pdf,1600,2000,"linear",linewidth=.8,colors=my_colors)
    utils.compare_mc(1764j,pdfs,"Pb214Bi214",order,1764j,pdf,1300,1600,"linear",linewidth=.8,colors=my_colors)

    utils.compare_mc(2204j,pdfs,"Pb214Bi214",order,1764j,pdf,1900,2500,"linear",linewidth=.8,colors=my_colors)
