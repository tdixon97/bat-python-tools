...
def get_quantiles(counts, bins):
    ...
    quantile_bins = [
        np.argmax(cumulative_counts >= q / 100 * total_count) for q in quantile_percentages
    ]
...
def run_example(N1, frac, MT2, MT1=44, label="Tl", make_plots=True, pdf=None):
    ...
    if make_plots:
        ...
        axes.set_title(f"{100-100*frac:.0f}% reduction, {MT2} kg-yr, {label}", fontsize=8)
...
if do:
    ...
    for type_analysis in ["Tl", "Bi"]:
        ### save the plots
        with PdfPages(f"plots/remove_ob_{type_analysis}_gamma.pdf") as pdf:
            ### run the exampls
            if type_analysis == "Tl":
            ...
            for frac in [0, 0.1, 0.2, 0.3, 0.5, 0.7, 0.8, 0.9, 1]:
                ...
                for exp_tmp in exp:
                    if type_analysis == "Tl":
                        n_ot, n_ob = run_example(
                            N_Tl, frac, exp_tmp, label="$^{208}$Tl (2615 keV)", make_plots=False
                        )
                    else:
                        n_ot, n_ob = run_example(
                            N_Bi, frac, exp_tmp, label="$^{214}$Bi (1764 keV)", make_plots=False
                        )
                ...
                axes.fill_between(
                    exp,
                    -np.array(low) + np.array(med),
                    np.array(high) + np.array(med),
                    color=vset.cyan,
                    alpha=0.4,
                    label="OB",
                    linewidth=0,
                ...
                axes.set_title(f"{100-100*frac:.0f}% reduction,  {label}", fontsize=8)

                axes.set_ylim(0, 1.6 * N / 44)
                axes.set_xlim(0.1, 20)
                axes.fill_between(
                    exp,
                    -np.array(low_ot) + np.array(med_ot),
                    np.array(high_ot) + np.array(med_ot),
                    color=vset.orange,
                    alpha=0.4,
                    label="other",
                    linewidth=0,
            ...
            for col, frac in zip(colors, [0.1, 0.3, 0.7, 0.9]):
                ...
                axes.fill_between(
                    ...
                    color=col,
                    alpha=0.3,
                    label=f"{100-100*frac:.0f}% reduce",
                    linewidth=0,
                )
                axes.plot(exp, med, color=col)
                axes.set_xlabel("Exposure [kg-yr]")
                axes.set_ylabel("$\mu_{ob}$ [kg-yr]")
                axes.set_title(f"OB rate {label}")
            ...
            for col, frac in zip(colors, [0.1, 0.3, 0.7, 0.9]):
                ...
                axes.set_title(f"Other rate {label}")
                axes.set_ylim(0, 1.2 * N / 44)
                axes.set_xlim(0.1, 20)
                axes.axhline(y=N / 44, color="b", linestyle="--")
                axes.legend(
...
for bi in [8e-4, 6e-4, 2e-4]:
    ...
    for exp_tmp in exp:
        if type_analysis == "bkg":
            n_ot = run_example_1D(
                5, frac, exp_tmp, label="$^{208}$Tl (2615 keV)", make_plots=False, MT1=31
            )
        med_ot.append(n_ot[0])
        low_ot.append(n_ot[1])
        high_ot.append(n_ot[2])
