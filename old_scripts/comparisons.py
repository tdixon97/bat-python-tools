...
parser.add_argument(
    ...
    type=str,
    help="file",
    default="../hmixfit/results/hmixfit-l200a-taup-silver-dataset-raw-histograms.root",
)
parser.add_argument(
    "-2",
    "--in_file_2",
    type=str,
    help="file",
    default="../hmixfit/results/hmixfit-l200a-taup-silver-dataset-m1-histograms.root",
)
parser.add_argument(
    "-c",
    "--components",
...
parser.add_argument(
    ...
    "--compare_data",
    type=int or bool,
    help="Compare data (true) or model (false)",
    default=True,
)
...
parser.add_argument(
    "-f", "--fit_name", type=str, help="fit name", default="l200a_vancouver23_dataset_v0_1"
)
...
if det_type == "multi" or det_type == "sum":
    datasets = [f"{fit_name}_bege", f"{fit_name}_icpc", f"{fit_name}_ppc", f"{fit_name}_coax"]
else:
    datasets = [f"{fit_name}_{det_type}"]
...
if det_type == "multi":
    fig, axes = lps.subplots(
        len(datasets), 1, figsize=(6, y), sharex=True, gridspec_kw={"hspace": 0}
    )
else:
    fig, axes_full = lps.subplots(
        2, 1, figsize=(6, y), sharex=True, gridspec_kw={"height_ratios": [8, 2], "hspace": 0}
    )
    ...
    with uproot.open(in_file_2) as f2:
        ...
        for i in range(len(datasets)):
            ...
            if ds == datasets[0] or det_type == "sum":
                ...
                ax.set_legend_logo(
                    position="upper right", logo_type="preliminary", scaling_factor=7
                )
            else:
                ax.xaxis.set_tick_params(top=False)
...
plt.tight_layout()
if compare_data == True:
    plt.savefig(f"plots/Comparison_{name_out_1}_to_{name_out_2}_{det_type}_data.pdf")
else:
    plt.savefig(f"plots/Comparison_{name_out_1}_to_{name_out_2}_{det_type}_model.pdf")
