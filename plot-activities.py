...
parser.add_argument(
    "-2", "--cfg_2", type=str, help="Second fit cfg file (to make comparison)", default=None
)
parser.add_argument(
    "-C",
    "--components",
...
parser.add_argument(
    ...
    "--obj",
    type=str,
    help="obj to plot 'fit_range' 'bi_range' or 'scaling_factor', 'parameter', 'eff_group'",
    default="fit_range",
)
parser.add_argument(
    "-t",
    "--type",
    type=str,
    help="Type for computation of fit/bi range either 'all', 'M1','M2' or 'by_spec'",
    default="all",
)
parser.add_argument(
    "-l",
    "--labels",
    type=str,
    help="A list of labels (comma sep.) for the comparison plots",
    default="fit1,fit2",
)
parser.add_argument(
...
parser.add_argument(
    "-s", "--save", type=int, help="save the plots? (1) or print to screen (0)?", default=True
)
parser.add_argument(
    "-e", "--energy", type=str, help="Energy range to use if the type if eff_groups", default=None
)
parser.add_argument(
    "-S", "--scale", type=str, help="Scale for the plots (default log)", default="log"
)
parser.add_argument(
    "-d", "--data_band", type=bool, help="Boolean flat to overlay a data band ", default=False
)
parser.add_argument(
    "-n", "--norm", type=bool, help="Normalise by observed counts in data ", default=False
)
...
if "livetime" in cfg:
    ...
    livetime = cfg["fit"]["theoretical-expectations"][dss[idx]][
        list(cfg["fit"]["theoretical-expectations"][dss[idx]].keys())[0]
    ]["livetime"]
    pdf_sub_path = cfg["fit"]["theoretical-expectations"][dss[idx]][
        list(cfg["fit"]["theoretical-expectations"][dss[idx]].keys())[0]
    ]["pdf-path"]
print(livetime)
...
if obj == "eff_group":
    ...
    for det_name, det_info in det_types.items():
        ...
        for det, named in zip(det_list, det_info["types"]):
            eff_new, good = utils.get_efficiencies(
                cfg, spectrum, det, regions, pdf_path, named, spectrum_fit=spectrum, type_fit="icpc"
            )
    ...
    for source in eff_total["all"]["peak"]:
        if "outer" in source:
            eff = eff_total["all"]["peak"][source] - (
                eff_total["all"]["left"][source] + eff_total["all"]["right"][source]
            ) * 15 / (2 * (energy_high - energy_low))
            print(f"Eficiency for {source} = {100*eff}")
if data_band == True:
    if obj == "eff_group":
        file = uproot.open(cfg["data-path"] + dss[idx])
        data_counts = utils.get_data_counts_total(
            spectrum, det_types, regions, file, key_list=["left", "right", "peak"]
        )
    else:
...
if obj == "scaling_factor" or obj == "parameter":
    list_of_plots = [{"name": "M1", "type": obj}]
...
path = f"{outdir}/{fit_name}/{obj}_{integral_type}_results.pdf"
...
with PdfPages(path) as pdf:
    for plot in list_of_plots:
        ...
        if type_plot != "eff_group":
            x, y, y_low, y_high, assay_norm = utils.get_data_numpy(
                type_plot, df_tot, name, type=types
            )
        else:
            x, y, y_low, y_high, assay_norm = utils.get_data_numpy(
                "parameter", df_tot, "M1", type=types
            )
        ...
        if type_plot == "eff_group":
            for id, n in enumerate(names):
                eff = eff_total["all"]["peak"][n] - (
                    eff_total["all"]["left"][n] + eff_total["all"]["right"][n]
                ) * 15 / (2 * (energy_high - energy_low))
                eff = max(eff, 0)
                y[id] *= eff
                y_low[id] *= eff
        ...
        if data_band is not False:
            band = utils.get_counts_minuit(
                np.array(
                    [
                        data_counts["all"]["left"],
        ...
        if do_comp == False:
            for ids in [np.arange(len(labels)), indexs["U"], indexs["Th"], indexs["K"]]:
                utils.make_error_bar_plot(
                    ...
                    split_priors=False,
                    has_prior=has_prior,
                    assay_mean=assay_mean,
                    assay_low=assay_low,
                    assay_high=assay_high,
                    scale=scale,
                    data_band=band,
                )
            ...
            if type_plot != "eff_group":
                x2, y2, y_low2, y_high2, assay_norm = utils.get_data_numpy(
                    type_plot, df_tot2, name, type=types
                )
            else:
                x2, y2, y_low2, y_high2, assay_norm = utils.get_data_numpy(
                    "parameter", df_tot2, "M1", type=types
                )
