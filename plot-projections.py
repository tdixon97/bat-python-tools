...
def main():
    ...
    parser.add_argument(
        ...
        "--det_type",
        type=str,
        help='which type of splitting to use for the data, either "all,types,string","chan" or "floor"',
        default="chan",
    )
    parser.add_argument(
        "-e", "--data_sel", type=str, help="Detector type to use default None", default=None
    )
    parser.add_argument(
        "-S", "--string_sel", type=int, help="string to select default None", default=None
    )
    parser.add_argument(
        "-c",
        "--cfg",
        type=str,
        help="Fit cfg file",
        default="../hmixfit/inputs/cfg/l200a-taup-silver-m1.json",
    )
    ...
    parser.add_argument(
        ...
        "--step",
        type=int,
        help="Number of steps of the markov chain to take ",
        default=1000000,
    )
    parser.add_argument(
    ...
    ### -----------------------------------------
    det_types, namet, Ns = utils.get_det_types(
        det_type, string_sel, det_type_sel=det_sel, level_groups="cfg/level_groups_Sofia.json"
    )
    ...
    if "pdf-path" in cfg["fit"]["theoretical-expectations"][dss[idx]]["mul_surv/icpc"]:
        pdf_path += (
            "/" + cfg["fit"]["theoretical-expectations"][dss[idx]]["mul_surv/icpc"]["pdf-path"]
        )
    ...
    ### list of regions
    with open("cfg/regions.json") as file:
    ...
    for det_name, det_info in det_types.items():
        ...
        for det, named in zip(det_list, det_info["types"]):
            eff_new, good = utils.get_efficiencies(
                cfg,
                spectrum,
                det,
                regions,
    ...
    if det_type == "all" or det_type == "types":
        tree = f"{fit_name}_mcmc"
        df = utils.ttree2df(outfile, "mcmc", "Phase==1", N=N)

        df = df.query("Phase==1")
        df = df.drop(
            columns=["Chain", "Iteration", "Phase", "LogProbability", "LogLikelihood", "LogPrior"]
        )
            ...
            for key, eff in effs.items():
                for group in groups:
                    ...
                    if group != "all":
                        ...
                        eff_tmp = {
                            key_e: value_e
                            for key_e, value_e in eff.items()
                            if key_e in filtered_columns
                        }
                    else:
                        eff_tmp = eff
        ...
        for key, eff in effs.items():
            for group in groups:
                if group == "all":
                    ...
                    eff_tmp = eff
                    total = np.sum(
                        [eff[name] * global_mode[name] for name in eff.keys() if group in name]
                    )
    ...
    if det_type == "all" or det_type == "types":
        ...
        with PdfPages(f"{outdir}/{fit_name}/region_breakdown.pdf") as pdf:
            roi_map = {}
            ...
            for det_name in det_types:
                ...
                for region in regions:
                    ...
                    fig, axes_full = lps.subplots(
                        1, 1, figsize=(4, 3), sharex=True, gridspec_kw={"hspace": 0}
                    )
                    maxi = 0
                    for group in groups:
                        print(group, region)
                        ...
                        axes_full.hist(
                            ...
                            alpha=0.3,
                            color=colors[group],
                            label=utils.format_latex([group])[0],
                        )
                        maxi = max(np.max(sums) * 1.1, maxi)
                        ...
                        quantiles = np.quantile(sums, [0.16, 0.84])
                        roi_map[det_name][region][group] = {
                            "mode": mode,
                            "q16": quantiles[0],
                            "q84": quantiles[1],
                            "exposure": exposure,
                        ...
                        axes_full.set_title(
                            f"Model reconstruction for {det_name} {region} {regions[region]!s} keV",
                            fontsize=8,
                        )
                ...
                with open(f"plots/summary/{fit_name}/{det_name}_bkg.json", "w") as json_file:
    ...
    for det_name, det_info in det_types.items():
        ...
        for det, type in zip(det_list, dt):
            if type == det_sel or det_sel == None:
                data_counts = utils.sum_effs(
                    data_counts, utils.get_data_counts(spectrum, det, regions, file)
                )
    ...
    with PdfPages(f"{outdir}/{fit_name}/observables_{det_type}.pdf") as pdf:
        for det_name in det_types:
            ...
            for key in effs.keys():
                ...
                if det_type == "chan":
                    continue
                rangef = (
                    int(min(np.min(data_real), data_counts[key])) - 0.5,
                    int(max(np.max(data_real), data_counts[key])) + 0.5,
                )
                ...
                axes_full.hist(
                    ...
                    alpha=0.3,
                    color=vset.red,
                    label="Expected realisations",
                )

                axes_full.set_xlabel("counts")
                axes_full.set_ylabel("Prob [arb]")
                axes_full.plot(
                    np.array([data_counts[key], data_counts[key]]),
                    np.array([axes_full.get_ylim()[0], axes_full.get_ylim()[1]]),
                    label="Data",
                    color="black",
                )
                axes_full.set_title(
                    f"Model reconstruction for {det_name} {key} {med:.2g}$^{{+{high:.2g}}}_{{-{low:.2g}}}$"
                )
    ...
    ### ---------------------------------------------------------------
    with PdfPages(f"{outdir}/{fit_name}/projections_{det_type}.pdf") as pdf:
        for region in summary:
            ...
            axes_full[0].set_title(f"Counts per group for {reg_fancy}")
            if det_type != "chan":
            ...
            for d, m, c in zip(datas, meds, names):
                if (d > 0) and (m == 0):
                    print(f"For {c} there are data events but not MC ")
            ...
            axes_full[1].errorbar(
                0.5 + np.arange(len(names)),
                residual,
                yerr=np.zeros(len(residual)),
                fmt="o",
                color="black",
                markersize=2,
            )
            ...
            if det_type != "chan":
                axes_full[1].set_xticks(
                    0.5 + np.arange(len(name_plot)), name_plot, rotation=80, fontsize=fontsize
                )
