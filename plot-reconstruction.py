...
parser.add_argument("-o","--originals",type=bool,help="Boolean flag to plot the original uniform binned histograms (in 1 keV binning)",default=False)
...
### -------------------------------------------------------
shutil.copy2(out_dir+"/hmixfit-"+fit_name+ "/know-update.pdf","{}/{}/".format(outdir,fit_name))
command = f"pdftk {outdir}/{fit_name}/know-update.pdf cat 1-{comps} output {outdir}/{fit_name}/tmp.pdf"
subprocess.run(command, shell=True)
subprocess.run(f"mv  {outdir}/{fit_name}/tmp.pdf {outdir}/{fit_name}/know-update.pdf", shell=True)
                        ...
                        if get_originals==True:
                            ...
                            if hs[comp] is None:
                                ...
                                if (dims==1):
                                    ...
                                    hs[comp]["2D_x"] += get_hist_rb(f[ds]["originals"][name]).project(0)
                                    hs[comp]["2D_y"] += get_hist_rb(f[ds]["originals"][name]).project(1)
                        else:
                            if name not in f[ds]:
                ...
                for type_plot in type_plots:
                    ...
                    if (shade_regions==True):
                        for region in regions.keys():
                            ...
                            axes_full[0].fill_between(np.array([range[0],range[1]]),np.array([max_y,max_y]),
                    ...
                    if (det_type!="multi"):
                        ...
                        if (exclude_range!=0):
                            axes_full[1].fill_between(np.array([exclude_range[0],exclude_range[1]]),y1=np.array([-6,-6]),y2=np.array([6,6]),
                                        alpha=1,color="grey",linewidth=0)
                    ...
                    for scale in ["linear","log"]:
                        ...
                        if (exclude_range!=0):
                            axes_full[0].fill_between(np.array([exclude_range[0],exclude_range[1]]),np.array([max_y_p,max_y_p]),
                                            label="Not in fit",alpha=1,color="grey",linewidth=0)
